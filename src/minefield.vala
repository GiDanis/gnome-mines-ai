...
public class Minefield : Object
{
    ...
    // Fixed mine layout for benchmark mode
    public bool use_fixed_layout = false;
    public int[]? fixed_mine_positions = null;
    ...
    {
        // Use fixed layout if enabled (benchmark mode)
        if (use_fixed_layout && fixed_mine_positions != null)
        {
            foreach (int pos in fixed_mine_positions)
            {
                // Validate position is within bounds
                if (pos < 0 || pos >= (int)(width * height))
                    continue;
                    
                int rx = pos % (int)width;
                int ry = pos / (int)width;
                
                // Skip first click cell
