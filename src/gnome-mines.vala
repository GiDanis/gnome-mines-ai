...
public class Mines : Adw.Application
{
    ...
    private AiSettingsFile ai_settings;
    ...
    // Benchmark mode fixed mine layouts for each difficulty
    // Small benchmark: 8x8, 10 mines (indices 0-63)
    private const int[] BENCHMARK_MINES_SMALL = {
        5, 12, 23, 34, 45, 18, 27, 38, 49, 56
    };
    // Medium benchmark: 16x16, 40 mines (indices 0-255)
    private const int[] BENCHMARK_MINES_MEDIUM = {
        18, 23, 45, 67, 89, 102, 134, 156, 178, 189,
        201, 223, 245, 35, 58, 91, 112, 143, 167, 203,
        28, 73, 95, 127, 149, 182, 214, 236, 41, 84,
        106, 138, 172, 195, 227, 249, 62, 118, 163, 231
    };
    // Large benchmark: 30x16, 99 mines (indices 0-479)
    private const int[] BENCHMARK_MINES_LARGE = {
        18, 23, 45, 67, 89, 102, 134, 156, 178, 189,
        201, 223, 245, 267, 289, 301, 323, 345, 367, 389,
        401, 423, 445, 467, 478, 15, 38, 61, 84, 107,
        130, 153, 176, 199, 222, 245, 268, 291, 314, 337,
        360, 383, 406, 429, 452, 12, 35, 58, 81, 104,
        127, 150, 173, 196, 219, 242, 265, 288, 311, 334,
        357, 380, 403, 426, 449, 472, 25, 48, 71, 94,
        117, 140, 163, 186, 209, 232, 255, 278, 301, 324,
        347, 370, 393, 416, 439, 462, 8, 31, 54, 77,
        100, 123, 146, 169, 192, 215, 238, 261, 284, 307
    };
    ...
    private AiPreferencesDialog? ai_preferences_dialog = null;
    private AiCommentaryOverlay? ai_commentary_overlay = null;
    ...
    private const GLib.ActionEntry[] action_entries =
    {
        ...
        { "benchmark-size",     benchmark_size_clicked_cb                   },
        { "benchmark-small",    benchmark_small_clicked_cb                  },
        { "benchmark-large",    benchmark_large_clicked_cb                  },
        { "cancel-custom",      show_new_game_screen                        },
        { "start-custom",       custom_size_clicked_cb                      },
        { "pause",              toggle_pause_cb                             },
        { "scores",             scores_cb                                   },
        { "preferences",        preferences_cb                              },
        { "ai-preferences",     ai_preferences_cb                           },
        { "ai-benchmarks",      ai_benchmarks_cb                            },
        { "ai-toggle",          ai_toggle_cb                                },
        { "quit",               quit_cb                                     },
        { "help",               help_cb                                     },
        { "about",              about_cb                                    },
        { "menu",               menu_cb                                     }
    ...
    private void create_window () {
        ...
        // Add AI commentary sidebar (always visible on right)
        ai_commentary_overlay = new AiCommentaryOverlay ();
        ai_commentary_overlay.set_visible(true);  // Always visible
        ai_commentary_overlay.set_vexpand(true);
        main_screen_layout.append(ai_commentary_overlay);
    ...
    private void startup_new_game_screen (Builder builder)
    {
        ...
        // Benchmark buttons with trophy icon
        button = (Button) builder.get_object ("benchmark_size_btn");
        label = new Label (null);
        label.set_markup ("<span size='xx-large' weight='heavy'>🏆</span>\nBenchmark\n<small>16×16, 40</small>");
        label.set_justify (Justification.CENTER);
        button.set_child (label);
        
        button = (Button) builder.get_object ("benchmark_small_btn");
        label = new Label (null);
        label.set_markup ("<span size='xx-large' weight='heavy'>🏆</span>\nBenchmark\n<small>8×8, 10</small>");
        label.set_justify (Justification.CENTER);
        button.set_child (label);
        
        button = (Button) builder.get_object ("benchmark_large_btn");
        label = new Label (null);
        label.set_markup ("<span size='xx-large' weight='heavy'>🏆</span>\nBenchmark\n<small>30×16, 99</small>");
        label.set_justify (Justification.CENTER);
        button.set_child (label);
    }
    ...
    private void start_game ()
    {
        ...
        int x, y, n;
        // Read mode directly, with fallback to game_mode variable
        int mode = settings.get_int (KEY_MODE);
        if (game_mode != -1)
            mode = game_mode;
        ...
        case 2:
            ...
            break;
        case 4:
            // Benchmark medium: fixed 16x16 board with 40 mines in fixed positions
            x = 16;
            y = 16;
            n = 40;
            break;
        case 5:
            // Benchmark small: fixed 8x8 board with 10 mines in fixed positions
            x = 8;
            y = 8;
            n = 10;
            break;
        case 6:
            // Benchmark large: fixed 30x16 board with 99 mines in fixed positions
            x = 30;
            y = 16;
            n = 99;
            break;
        ...
        // Set fixed mine layout for benchmark modes
        if (mode == 4)
        {
            minefield.use_fixed_layout = true;
            minefield.fixed_mine_positions = BENCHMARK_MINES_MEDIUM;
        }
        else if (mode == 5)
        {
            minefield.use_fixed_layout = true;
            minefield.fixed_mine_positions = BENCHMARK_MINES_SMALL;
        }
        else if (mode == 6)
        {
            minefield.use_fixed_layout = true;
            minefield.fixed_mine_positions = BENCHMARK_MINES_LARGE;
        }
        ...
        // Update AI manager with new minefield
        if (ai_manager != null)
        {
            ai_manager.stop();
            ai_manager.update_minefield(minefield);
        }
        
        // Clear AI commentary on new game
        if (ai_commentary_overlay != null)
        {
            ai_commentary_overlay.clear();
            ai_commentary_overlay.add_message("🎮 Nuova partita iniziata!", "info");
        }
    ...
    {
        // Save mode immediately using gsettings with sync
        settings.set_int (KEY_MODE, mode);
        settings.sync();  // Force save
        
        // Start game directly with this mode
        game_mode = mode;
        if (window == null)
            create_window ();
    ...
    private void benchmark_size_clicked_cb ()
    {
        set_mode (4);  // Mode 4 = benchmark medium (16x16)
    }
    
    private void benchmark_small_clicked_cb ()
    {
        set_mode (5);  // Mode 5 = benchmark small (8x8)
    }
    
    private void benchmark_large_clicked_cb ()
    {
        set_mode (6);  // Mode 6 = benchmark large (30x16)
    }
    ...
    private void ai_preferences_cb ()
    {
        if (ai_preferences_dialog == null)
        {
            ai_preferences_dialog = new AiPreferencesDialog (window, settings);
        }
        ai_preferences_dialog.present (window);
    }
    
    private void ai_benchmarks_cb ()
    {
        var dialog = new AiBenchmarkDialog(window);
        dialog.present(window);
    }
    
    public void notify_ai_model_changed(string new_model)
    {
        // Update AI commentary overlay model label
        if (ai_commentary_overlay != null)
        {
            ai_commentary_overlay.set_model(new_model);
        }
    }

    private void ai_toggle_cb ()
    {
        // Initialize AI manager if not already done
        if (ai_manager == null && minefield != null)
        {
            ai_manager = new AiManager (minefield, settings, ai_commentary_overlay);
            ai_manager.commentary_ready.connect((text, type) => {
                if (ai_commentary_overlay != null)
                    ai_commentary_overlay.add_message(text, type);
            });
            ai_manager.ai_state_changed.connect((active, thinking) => {
                if (thinking && ai_commentary_overlay != null)
                    ai_commentary_overlay.add_message("🤖 AI sta pensando...", "think");
                
                // Update menu item state
                var action = lookup_action("ai-toggle") as SimpleAction;
                if (action != null)
                    action.set_state(new GLib.Variant.boolean(active));
                
                // Update model label every time AI state changes
                if (ai_commentary_overlay != null)
                {
                    string current_model = settings.get_string("ai-model");
                    if (current_model != "")
                        ai_commentary_overlay.set_model(current_model);
                }
            });
        }
        ...
        if (ai_manager != null)
        {
            if (minefield == null)
            {
                ai_commentary_overlay.add_message("⚠️ Inizia prima una partita!", "error");
                return;
            }
            
            ai_manager.active = !ai_manager.active;
            
            if (ai_manager.active)
            {
                ai_manager.initialize_provider();
                if (ai_commentary_overlay != null)
                {
                    ai_commentary_overlay.clear();
                    ai_commentary_overlay.add_message("🤖 AI attivata!", "info");
                    ai_commentary_overlay.add_message("🎮 Inizio analisi...", "think");
                    // Update model label
                    string current_model = settings.get_string("ai-model");
                    if (current_model != "")
                        ai_commentary_overlay.set_model(current_model);
                ...
                Timeout.add(1000, () => {
                    if (ai_manager != null && ai_manager.active)
                        ai_manager.think();
            ...
            else
            {
                ai_manager.stop();
                if (ai_commentary_overlay != null)
                    ai_commentary_overlay.add_message("⏸️ AI disattivata", "info");
            }
        }
    }
